        ...
        <form action="{{ route('Issues.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3 form-group">
                ...
                <select class="form-control @error('computer_id') is-invalid @enderror" name="computer_id" id="computer_id">
                    ...
                    <option value="{{ $item->id }}" {{ old('computer_id') == $item->id ? 'selected' : '' }}>
                        {{ $item->computer_name }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="mb-3 form-group">
                ...
                <input class="form-control @error('reporter_date') is-invalid @enderror" type="datetime-local" name="reporter_date" id="reporter_date" value="{{ old('reporter_date') }}">
                @error('reporter_date')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
